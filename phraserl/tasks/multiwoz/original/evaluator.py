"""
Original code:
https://github.com/budzianowski/multiwoz/blob/master/evaluate.py
"""
...
from . import delexicalizer as delex
from .normalizer import normalize

DELEX_FILE_PATH = "data/norm-multi-woz/delex.json"
...
class MultiWozDB(object):
    ...
    for domain in domains:
        db = os.path.join(delex.DB_DIR, f"{domain}-dbase.db")
...
class OrigMultiWozEvaluator(BaseEvaluator):
    def __init__(self):
        self.slot_dict = delex.prepareSlotValuesIndependent()
        self.delex_dialogues = json.load(open(DELEX_FILE_PATH))
        self.db = MultiWozDB()
        self.labels = list()
        self.hyps = list()
    ...
    def _evaluateGeneratedDialogue(
        ...
        for t, sent_t in enumerate(dialog):
            for domain in goal.keys():
                ...
                # ATTENTION: assumption here
                # e.g. we didn't provide phone or address twice!
    ...
    def evaluateModel(self, dialogues, soft_acc=False):
        """
        Gathers statistics for the whole sets.
        Args:
            dialogues: dict {"filename": [system utterances]}
        """
        ...
        for filename, dial in dialogues.items():
            ...
            success, match, stats = self._evaluateGeneratedDialogue(
                dial, goal, data, requestables, soft_acc=soft_acc
            )
        ...
        return successes / float(total), matches / float(total)
